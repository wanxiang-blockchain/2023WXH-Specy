...
// StartListener starts the main relaying loop and returns a channel that will contain any control-flow related errors.
func StartListener(
	ctx context.Context,
	log *zap.Logger,
	chain *Chain,
	...
	chainProcessors := make([]processor.ChainProcessor, 0, 1)

	chainProcessors = append(chainProcessors, chain.chainProcessor(log))

	go listenerStartEventProcessor(
		ctx,
		chainProcessors,
		initialBlockHistory,
		messageLifecycle,
...
// TODO: intermediate types. Should combine/replace with the scheduler.Chain, scheduler.Path, and scheduler.PathEnd structs
...
func (chain *Chain) chainProcessor(log *zap.Logger) processor.ChainProcessor {
...
// listenerStartEventProcessor is the main relayer process when using the event processor.
func listenerStartEventProcessor(
	ctx context.Context,
