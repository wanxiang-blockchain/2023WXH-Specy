...
export default function useCosmosBankV1Beta1() {
  ...
  const QueryTotalSupplyWithoutOffset = (query: any, options: any, perPage: number) => {
    const key = { type: 'QueryTotalSupplyWithoutOffset', query };    
    return useInfiniteQuery([key], ({pageParam = 1}: { pageParam?: number}) => {
      const {query } = key
      ...
      query['pagination.offset']= (pageParam-1)*perPage;
      query['pagination.count_total']= true;
      return  client.CosmosBankV1Beta1.query.queryTotalSupplyWithoutOffset(query ?? undefined).then( res => ({...res.data,pageParam}) );
    }, {...options,
      getNextPageParam: (lastPage, allPages) => { if ((lastPage.pagination?.total ?? 0) >((lastPage.pageParam ?? 0) * perPage)) {return lastPage.pageParam+1 } else {return undefined}},
      getPreviousPageParam: (firstPage, allPages) => { if (firstPage.pageParam==1) { return undefined } else { return firstPage.pageParam-1}}
    }
  ...
  const QuerySupplyOfWithoutOffset = (denom: string,  options: any) => {
    const key = { type: 'QuerySupplyOfWithoutOffset',  denom };    
    return useQuery([key], () => {
      const { denom } = key
      return  client.CosmosBankV1Beta1.query.querySupplyOfWithoutOffset(denom).then( res => res.data );
    }, options);
  }
  ...
  const QueryBaseDenom = (query: any, options: any) => {
    const key = { type: 'QueryBaseDenom', query };    
    return useQuery([key], () => {
      const {query } = key
      return  client.CosmosBankV1Beta1.query.queryBaseDenom(query ?? undefined).then( res => res.data );
    }, options);
  }
  
  return {QueryBalance,QueryAllBalances,QueryTotalSupply,QuerySupplyOf,QueryTotalSupplyWithoutOffset,QuerySupplyOfWithoutOffset,QueryParams,QueryDenomMetadata,QueryDenomsMetadata,QueryBaseDenom,
  }
}