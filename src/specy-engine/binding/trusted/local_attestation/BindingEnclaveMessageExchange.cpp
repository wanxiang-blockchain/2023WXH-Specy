...
// Author:
//
// This file implements functions for local attestation.

#include "BindingEnclaveMessageExchange.h"

using namespace std;

// Function pointer table containing the list of functions that the enclave exposes
// For now, there is no exported function in binding enclave
const struct
{
    size_t num_funcs;
    const void *table[2];
                                       ...
                                       sgx_enclave_id_t dest_enclave_id)
{
    ...
    ocall_print_string("enter Binding::ecall enclave_to_enclave_call", __FILE__, __LINE__);
                                                       ...
                                                       size_t *resp_length)
{
    ...
    uint32_t err = fn1(ms, decrypted_data_length, resp_buffer, resp_length);
    ocall_print_string("exit enclave_to_enclave_call", __FILE__, __LINE__);
