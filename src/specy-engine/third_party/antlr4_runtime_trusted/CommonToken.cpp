...
//  return ss.str();
  ...
  // Re-write to fit in SGX Enclave runtime
  std::string ss;

  ss += "[@";
  ss += std::to_string(symbolToNumeric(getTokenIndex()));
  ss += ",";
  ss += std::to_string(symbolToNumeric(_start));
  ss += ":";
  ss += std::to_string(symbolToNumeric(_stop));
  ss += "='";
  ss += txt;
  ss += "',<";
  ss += typeString;
  ...
  ss += channelStr;
  ss += ",";
  ss += std::to_string(_line);
  ss += ":";
  ss += std::to_string(getCharPositionInLine());
  ss += "]";
