...
void ANTLRInputStream::load(const std::string &input) {
  ...
  if (input.compare(0, 3, bom, 3) == 0){
  	std::u32string s32(input.begin() + 3, input.end());
	        _data = s32;
  }
  else{
  	std::u32string s32(input.begin(), input.end());
	        _data = s32;
  }
  p = 0;
}
...
std::string ANTLRInputStream::getText(const Interval &interval) {
  ...
  std::string res;
    for (auto c = _data.begin()+start; c != _data.begin()+start+count; ++c){
	        char t = *c;
		    res.push_back(t);
		      }
...
std::string ANTLRInputStream::toString() const {
	std::string res;
	  for (auto c = _data.begin(); c != _data.end(); ++c){
		      char t = *c;
		          res.push_back(t);
			    }
